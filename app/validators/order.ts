 ...
 */
export const createOrderValidator = (companyId: number, locationId: number) => vine.compile(
  vine.object({
 ...
 */
export const updateOrderValidator = (companyId: number, locationId: number) => vine.compile(
  vine.object({
 ...
 * lo que cambia (ej. solo el estado).
 */
export const cancelOrderValidator = (companyId: number) => vine.compile(
  vine.object({
    // El método de pago es opcional, pero podría ser necesario para el reembolso
    paymentMethodId: vine
      .number()
      .exists(async (db, value) => {
        return await db.from('payment_methods')
          .where('id', value)
          .where('company_id', companyId)
          .first()
      }).optional(),
