...
import { closeCashRegisterSessionValidatorWithCompany, createCashRegisterSessionValidatorWithCompany } from '#validators/cash_register_session'
...
export default class CashRegisterSessionsController {
  ...
  async index({ response, companyId, getQueryData }: HttpContext) {
    ...
    try {
      const cashRegisters = await CashRegisterSession.withCompanyFilter(companyId)
   ...
   */
  async store({ request, response, auth, companyId, currentUser }: HttpContext) {
    const trx = await db.transaction()
    try {
      // Crear el validador con el companyId del contexto
      const validator = createCashRegisterSessionValidatorWithCompany(companyId)
      ...
      const cashRegister = await CashRegister.findOrFail(payload.cashRegisterId)
      ...
      const existingOpenSessionInLocation = await CashRegisterSession.query({ client: trx })
        .where('status', 'open')
      ...
      const cashRegisterSession = await CashRegisterSession.create(cashRegisterSessionData)
  ...
  async show({ params, response, companyId }: HttpContext) {
    try {
      const cashRegisterSession = await CashRegisterSession.withCompanyFilter(companyId)
        .where('id', params.id)
        .firstOrFail()
   ...
   */
  async closeSession({ request, response, params, companyId, locationId }: HttpContext) {
    ...
    const trx = await db.transaction()
    try {
      const cashRegisterSession = await CashRegisterSession.query({ client: trx })
        .where('id', params.id)
      ...
      if (cashRegisterSession.status === 'closed') {
        await trx.rollback()
        return response.conflict({ message: 'Esta sesión de caja ya ha sido cerrada.' });
      }
      ...
      const movements = await CashMovement.query({ client: trx })
        .where('cash_register_session_id', params.id)
      ...
      const calculatedClosingBalance = openingBalance.plus(totalSales).minus(totalWithdrawals);
      const realClosingBalance = new Decimal(payload.realClosingBalance);
      const differenceAmount = realClosingBalance.minus(calculatedClosingBalance);
      ...
      const updateData = {
        ...
        closedAt: payload.closedAt ? DateTime.fromJSDate(payload.closedAt) : DateTime.now()
      }
      ...
      io.to(roomName).emit('cash_register_closed', cashRegisterSession.serialize())
      ...
      if (error.code === 'E_ROW_NOT_FOUND') {
        return response.notFound({ message: 'La sesión de caja no fue encontrada.' })
      }
      return response.internalServerError({
  ...
  async getSummary({ response, params, companyId }: HttpContext) {
    try {
      ...
      const incomeSummary = await db.from('order_payments')
      ...
      const salesResult = await db.from('orders')
        .where('cash_register_session_id', params.id)
        .where('status', 'paid')
      ...
      const totalWithdrawals = new Decimal(withdrawalsResult?.$extras.total || 0);
      ...
      const expectedClosingBalance = openingBalance.plus(totalSales).minus(totalWithdrawals);
      ...
      if (error.code === 'E_ROW_NOT_FOUND') {
        return response.notFound({
          message: 'La sesión de caja no fue encontrada o no pertenece a tu compañía.'
        })
      }
   ...
   */
  public async getDailyBusinessSummary({ response, companyId, locationId, cashRegisterSessionId }: HttpContext) {
    try {
      ...
      const [
        ...
        db.from('order_items')
          ...
          .select('products.name', 'categories.id as categoryId', 'categories.name as categoryName')
      ...
      const summaryData = {
        totalRevenue: new Decimal(totalMetrics?.$extras.totalRevenue || 0).toNumber(),
